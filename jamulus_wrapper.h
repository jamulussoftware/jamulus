...
extern "C"
{
    ...
    typedef struct
    {
        ...
        char players[2048]; // Newline-separated list of players
...
#define JAMULUS_DIRECTORY_CUSTOM          7

    // Request server list from a directory
    void jamulus_client_request_server_list ( jamulus_client_t c, int directory_type );

    // Request list of connected clients for a specific server (by address)
    void jamulus_client_request_server_client_list ( jamulus_client_t c, const char* serverAddress );

    // Set a custom directory address used when directory_type NB: JAMULUS_DIRECTORY_CUSTOM
    // Format: host:port (e.g., mydir.example.com:22124)
    void jamulus_client_set_custom_directory_address ( const char* directoryAddress );

    // Send a single ping (with client count request) to the given server address (host:port or [ipv6]:port)
    void jamulus_client_ping_single ( jamulus_client_t c, const char* serverAddress );
    ...
    const char* jamulus_get_log_file_path ( void );
