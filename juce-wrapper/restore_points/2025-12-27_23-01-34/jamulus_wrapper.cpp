...
extern "C"
{
    ...
    const char* jamulus_client_get_country_code ( jamulus_client_t c )
    {
        ...
        // Convert QLocale::Country to ISO alpha-2 code
        QString    name = QLocale ( QLocale::AnyLanguage, client->ChannelInfo.eCountry ).name();
        QByteArray code = name.contains ( '_' ) ? name.split ( '_' ).last().toLower().toUtf8() : name.toLower().toUtf8();
    ...
    void jamulus_client_set_channel_solo ( jamulus_client_t c, int channel_id, bool solo )
    {
        ...
        // Track solo state locally. In a real mixer, soloing one or more channels
        // usually mutes all others.
