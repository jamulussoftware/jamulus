    ...
    : AudioProcessorEditor(&p), audioProcessor(p)
{
    ...
    if (client)
    {
        ...
        guiComponent->onSidechainChanged = [this](bool enabled) {
            audioProcessor.setSidechainEnabled(enabled);
        };
        guiComponent->setSidechainAvailable(audioProcessor.isSidechainAvailable());
        guiComponent->setTestToneButtonCallback([this](bool enabled) {
            audioProcessor.getProcessor().setTestToneEnabled(enabled);
        });
