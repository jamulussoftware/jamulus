...
public:
    ...
    int ProcessInterleavedFloat(const float* in, float* out, int frames, int channels)
    {
        ...
        for (int i = 0; i < totalSamples; ++i)
        {
            ...
            {
                out[i] = internalOutputBuffer[outputReadPos % internalOutputBuffer.size()];
                outputReadPos++;
            }
            else
            {
...
static void updatePingTime(int pingTime)
{
    s_lastPingTime = pingTime;
}
    ...
    for (int i = 0; i < vecServerInfo.Size(); ++i)
    {
        ...
        for (auto& existing : s_serverList)
        {
            if (strcmp(existing.address, srvInfo.address) == 0)
            {
                ...
                existing = srvInfo;
                if (oldPing >= 0) existing.ping = oldPing;
                if (oldNumClients > 0) existing.numClients = oldNumClients;
                found = true;
                break;
            }
        }
...
void jamulus_client_set_country(jamulus_client_t c, int country)
{
    if (!c) return;
    CClient* client = reinterpret_cast<CClient*>(c);
    client->ChannelInfo.eCountry = static_cast<QLocale::Country>(country);
    client->SetRemoteInfo();
}
...
int jamulus_client_get_country(jamulus_client_t c)
{
    if (!c) return 0;
    CClient* client = reinterpret_cast<CClient*>(c);
    return static_cast<int>(client->ChannelInfo.eCountry);
}
...
int jamulus_client_get_overall_delay(jamulus_client_t c)
{
    if (!c) return -1;
    CClient* client = reinterpret_cast<CClient*>(c);
    if (!client->IsConnected() || s_lastPingTime < 0) return -1;
...
{
    if (!c) return -1;
    // Return the server-assigned ID directly. The GUI compares this against 
    // the iChanID field in channel info to identify 'Me'.
...
bool jamulus_client_get_channel_info(jamulus_client_t c, int index, jamulus_channel_info_t* info)
{
    ...
    QMutexLocker lock(&s_channelMutex);
    if (index < 0 || index >= s_channelList.Size()) return false;
    ...
    info->gain = 1.0f;  // TODO: get from stored fader values
    info->pan = 0.0f;
    info->muted = false;
    info->solo = false;
...
void jamulus_client_set_channel_gain(jamulus_client_t c, int channel_id, float gain)
{
    if (!c) return;
    CClient* client = reinterpret_cast<CClient*>(c);
    // Gain 0.0-1.0 -> 0.0-1.0 for Jamulus (already normalized)
    client->SetRemoteChanGain(channel_id, gain, false);
}
...
void jamulus_client_set_channel_pan(jamulus_client_t c, int channel_id, float pan)
{
    if (!c) return;
    CClient* client = reinterpret_cast<CClient*>(c);
    // Pan -1.0 to 1.0 -> 0.0 to 1.0 for Jamulus
    float jamulusPan = (pan + 1.0f) * 0.5f;
    client->SetRemoteChanPan(channel_id, jamulusPan);
}
...
void jamulus_client_set_channel_mute(jamulus_client_t c, int channel_id, bool muted)
{
    ...
    if (muted) {
        client->SetRemoteChanGain(channel_id, 0.0f, false);
    }
    // Note: unmute would need to restore previous gain, not implemented yet
}

void jamulus_client_set_channel_solo(jamulus_client_t c, int channel_id, bool solo)
{
    // Solo is more complex - would need to mute all others
    // Not fully implemented for now
    (void)c; (void)channel_id; (void)solo;
}
