    ...
    void paint(juce::Graphics& g) override
    {
        ...
        for (const auto& msg : messages)
        {
            ...
            if (msg.isSystemMessage)
            {
                ...
                auto attrStr = createAttributedString("* " + msg.text, juce::Colour(0xff888888));
                juce::TextLayout layout;
                layout.createLayout(attrStr, static_cast<float>(getWidth() - padding * 2));
                layout.draw(g, juce::Rectangle<float>(static_cast<float>(padding), 
                                                       static_cast<float>(y), 
            ...
            else
            {
                ...
                g.drawText(header, padding, y, getWidth() - padding * 2, 18,
                           juce::Justification::centredLeft);
                
                y += 18;
                
                // Message text with HTML support
                auto attrStr = createAttributedString(msg.text, juce::Colours::white);
                juce::TextLayout layout;
                layout.createLayout(attrStr, static_cast<float>(getWidth() - padding * 2));
                layout.draw(g, juce::Rectangle<float>(static_cast<float>(padding), 
                                                       static_cast<float>(y), 
...
private:
    juce::AttributedString createAttributedString(const juce::String& text, juce::Colour defaultColour)
    {
        ...
        while (pos < text.length())
        {
            ...
            if (tagStart < 0)
            {
                attr.append(text.substring(pos), juce::Font(13.0f, boldStack.back() ? juce::Font::bold : juce::Font::plain), colourStack.back());
                break;
            }
            
            if (tagStart > pos)
                attr.append(text.substring(pos, tagStart), juce::Font(13.0f, boldStack.back() ? juce::Font::bold : juce::Font::plain), colourStack.back());
            
            int tagEnd = text.indexOf(tagStart, ">");
            if (tagEnd < 0)
            {
                attr.append(text.substring(tagStart), juce::Font(13.0f, boldStack.back() ? juce::Font::bold : juce::Font::plain), colourStack.back());
                break;
            }
            ...
            if (tag.startsWithIgnoreCase("<font"))
            {
                ...
                if (colorPos > 0)
                {
                    ...
                    int colorValEnd = tag.indexOf(colorValStart, (quoteChar == '\"' || quoteChar == '\'') ? juce::String::charToString(quoteChar) : " >");
                    ...
                    if (colorValEnd > colorValStart)
                    {
                        ...
                        if (colorStr.startsWith("#"))
                        {
                            ...
                            if (hex.length() == 6)
                                ...
                                newColour = juce::Colour::fromString("ff" + juce::String::charToString(hex[0]) + juce::String::charToString(hex[0]) + 
            ...
            {
                if (colourStack.size() > 1) colourStack.pop_back();
            }
            ...
            {
                if (boldStack.size() > 1) boldStack.pop_back();
            }
...
{
public:
    ChatComponent(jamulus_client_t client, bool panelMode = false)
        : jamulusClient(client), isPanelMode(panelMode)
    {
    ...
    void resized() override
    {
        ...
        if (!isPanelMode)
        {
            ...
            clearButton.setBounds(buttonArea.removeFromRight(80));
        }
        ...
        messageList.setSize(messageViewport.getWidth() - messageViewport.getScrollBarThickness(), 
...
public:
    ChatWindow(jamulus_client_t client)
        : DocumentWindow("Jamulus Chat", juce::Colour(0xff323232), 
