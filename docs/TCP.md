...
It's also possible a large welcome message or chat text could require fragmentation, as `MAX_LEN_CHAT_TEXT` is 1600 bytes and there is Jamulus protocol, UDP and IP overhead above that. This might be mitigated by Jamulus's split message protocol, although that currently does not get negotiated until after the welcome message is sent. Also, the split message protocol does not apply to connectionless messages, only connected-mode.
