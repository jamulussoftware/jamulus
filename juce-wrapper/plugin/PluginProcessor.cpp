...
void JamulusPluginProcessor::processBlock(const float** inputs, float** outputs, int numChannels, int numSamples)
{
    ...
    if (client)
    {
        if (needsResampling)
        {
            ...
            int jamulusFrames = inputResampler.resampleTo48k(interleavedIn.data(), numSamples, 
                                                              jamulusIn.data(), maxJamulusFrames);

            // Process at 48kHz
            int rc = jamulus_client_process_audio(client, jamulusIn.data(), jamulusOut.data(), 
                                                   jamulusFrames, numChannels);
            if (rc != 0)
            {
            ...
            outputResampler.resampleFrom48k(jamulusOut.data(), jamulusFrames,
                                            interleavedOut.data(), numSamples);
        }
        else
        {
            // No resampling needed - direct processing at 48kHz
            int rc = jamulus_client_process_audio(client, interleavedIn.data(), interleavedOut.data(), 
