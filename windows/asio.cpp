...
{
	return pAsioDriver->start();
}

ASIOError ASIOStop(void)
{
	return pAsioDriver->stop();
}
...
{
	return pAsioDriver->getLatencies( inputLatency, outputLatency );
}
...
{
	return pAsioDriver->canSampleRate( sampleRate );
}

ASIOError ASIOGetSampleRate( ASIOSampleRate *currentRate )
{
	return pAsioDriver->getSampleRate( currentRate );
}

ASIOError ASIOSetSampleRate( ASIOSampleRate sampleRate )
{
	return pAsioDriver->setSampleRate( sampleRate );
}

ASIOError ASIOGetClockSources( ASIOClockSource *clocks, long *numSources )
{
	return pAsioDriver->getClockSources( clocks, numSources );
}

ASIOError ASIOSetClockSource( long reference )
{
	return pAsioDriver->setClockSource( reference );
}

ASIOError ASIOGetSamplePosition( ASIOSamples *sPos, ASIOTimeStamp *tStamp )
{
	return pAsioDriver->getSamplePosition( sPos, tStamp );
}

ASIOError ASIOGetChannelInfo( ASIOChannelInfo *info )
{
	return pAsioDriver->getChannelInfo(info);
}
...
{
	return pAsioDriver->disposeBuffers();
}

ASIOError ASIOControlPanel(void)
{
	return pAsioDriver->controlPanel();
}

ASIOError ASIOFuture( long selector, void *opt )
{
	return pAsioDriver->future( selector, opt );
}

ASIOError ASIOOutputReady(void)
{
	return pAsioDriver->outputReady();
}
