...
static juce::Image getInstrumentImage(int instrument) {
    static const char* instrumentFiles[] = {
        "none.png", "drumset.png", "djembe.png", "eguitar.png", "aguitar.png",
        "bassguitar.png", "keyboard.png", "synthesizer.png", "grandpiano.png", "accordeon.png",
        "vocal.png", "microphone.png", "harmonica.png", "trumpet.png", "trombone.png",
        "frenchhorn.png", "tuba.png", "saxophone.png", "clarinet.png", "flute.png",
        "violin.png", "cello.png", "doublebass.png", "recorder.png", "streamer.png",
        "listener.png", "guitarvocal.png", "keyboardvocal.png", "bodhran.png", "bassoon.png",
        "oboe.png", "harp.png", "viola.png", "congas.png", "bongo.png",
        "vocalbass.png", "vocaltenor.png", "vocalalto.png", "vocalsoprano.png", "banjo.png",
        "mandolin.png", "ukulele.png", "bassukulele.png", "vocalbaritone.png", "vocallead.png",
        "mountaindulcimer.png", "scratching.png", "rapping.png", "vibraphone.png", "conductor.png"
    };
...
static juce::Image getFlagImage(const char* countryCode) {
    ...
    // Some common fixes
    if (code.equalsIgnoreCase("uk")) code = "gb";
...
public:
    ...
    void paint(juce::Graphics& g) override
    {
        ...
        juce::Rectangle<float> levelRect(
            bounds.getX() + 1,
            bounds.getBottom() - levelHeight - 1,
            bounds.getWidth() - 2,
            levelHeight
        );
...
public:
    ChannelStrip()
    {
        ...
        headerArea.setInterceptsMouseClicks(false, false);
        ...
        skillLabel.setFont(juce::Font(10.0f));
        skillLabel.setJustificationType(juce::Justification::topRight);
    ...
    void updateMuteButtonColor()
    {
        if (muteButton.getToggleState())
            muteButton.setColour(juce::TextButton::buttonColourId, juce::Colours::red.withAlpha(0.8f));
        else
    ...
    void updateSoloButtonColor()
    {
        if (soloButton.getToggleState())
            soloButton.setColour(juce::TextButton::buttonColourId, juce::Colours::orange.withAlpha(0.8f));
        else
    ...
    void setChannelInfo(int id, const juce::String& name, int instrument, const char* countryCode, int skill, bool isMe)
    {
        ...
        switch (skill) {
            case 1: skillText = "Beginner"; break;
            case 2: skillText = "Intermediate"; break;
            case 3: skillText = "Expert"; break;
            default: skillText = ""; break;
        }
    ...
    void paint(juce::Graphics& g) override
    {
        ...
        switch (skillLevel) {
            case 1: skillColour = juce::Colour(255, 255, 200); break; // Beginner: Pale Yellow
            case 2: skillColour = juce::Colour(225, 255, 225); break; // Intermediate: Pale Green
            case 3: skillColour = juce::Colour(255, 225, 225); break; // Professional: Pale Pink
            default: skillColour = juce::Colours::white; break;
        }
        ...
        // Flag Icon (top left of tag)
        if (flagIcon.isValid()) {
            g.drawImageWithin(flagIcon,
                             tagArea.getX() + 4, tagArea.getY() + 4, 20, 14,
                             juce::RectanglePlacement::centred);
        }
        
        // Instrument Icon (below flag)
        if (instrumentIcon.isValid()) {
            g.drawImageWithin(instrumentIcon, 
                             tagArea.getX() + 2, tagArea.getY() + 22, 24, 24,
                             juce::RectanglePlacement::centred);
        }
    }
    ...
    void resized() override
    {
        ...
        // Name on the right of the icons
        nameLabel.setBounds(tagArea.withTrimmedLeft(30).withTrimmedTop(2));
        
        // Skill level tag (bottom right of tag area)
        skillLabel.setBounds(tagArea.withSize(tagArea.getWidth() - 34, 14).withPosition(30, tagArea.getBottom() - 16));
        
        bounds.removeFromTop(5);
        
        // Bottom controls (Solo, Mute)
        auto bottomArea = bounds.removeFromBottom(60);
        soloButton.setBounds(bottomArea.removeFromTop(28).reduced(5, 2));
        muteButton.setBounds(bottomArea.removeFromTop(28).reduced(5, 2));
        
        bounds.removeFromBottom(5);
        
        // Pan
        panSlider.setBounds(bounds.removeFromBottom(30).reduced(15, 0));
        
        bounds.removeFromTop(5);
        
        // Fader and meter
        auto faderArea = bounds;
        levelMeter.setBounds(faderArea.removeFromRight(10));
...
public:
    ...
    void setTestToneButtonCallback(std::function<void(bool)> cb)
    {
        testToneButton.onClick = [this, cb]() {
            ...
            if (cb) cb(testToneEnabled);
        };
    }
    JamulusGuiComponent(jamulus_client_t client)
    ...
    void resized() override
    {
        ...
        // Chat panel on right side (if visible)
        const int chatPanelWidth = 250;
        if (chatPanelVisible && chatPanel)
        {
            chatPanel->setBounds(bounds.removeFromRight(chatPanelWidth));
        }
        ...
        // Controls area
        auto controlsArea = bounds.removeFromTop(220);
        
        // Left side - input meters and fader
        auto leftControls = controlsArea.removeFromLeft(200);
        
        auto inputArea = leftControls.removeFromTop(70);
        inputLevelLabel.setBounds(inputArea.removeFromTop(20));
        auto metersArea = inputArea.reduced(10, 0);
        inputMeterL.setBounds(metersArea.removeFromLeft(metersArea.getWidth() / 2 - 2));
        metersArea.removeFromLeft(4);
        inputMeterR.setBounds(metersArea);
        
        auto faderArea = leftControls.removeFromTop(70);
        inputFaderLabel.setBounds(faderArea.removeFromLeft(50).reduced(5));
        inputFaderSlider.setBounds(faderArea.removeFromTop(30).reduced(5));
        fxButton.setBounds(faderArea.removeFromLeft(40).reduced(5));
        sidechainButton.setBounds(faderArea.removeFromLeft(40).reduced(5));
        
        // Network Stats
        auto statsArea = leftControls.reduced(10, 5);
        int statHeight = 18;
        
        // Ping
        pingLabel.setBounds(statsArea.removeFromTop(statHeight));
        ...
        {
            fxPanel->setBounds(10, 230, 320, 530); // Adjusted Y position
        }
...
private:
    ...
    void timerCallback() override
    {
        ...
        if (connected != wasConnected)
        {
            ...
            statusLabel.setColour(juce::Label::textColourId, 
                                  connected ? juce::Colour(0xff00cc66) : juce::Colours::grey);
        ...
        if (connected)
        {
            ...
            if (stats.ping_time_ms >= 0)
            {
                ...
                if (stats.total_delay_ms <= 43) delayColor = juce::Colour(0xff00cc66); // Green
                else if (stats.total_delay_ms <= 68) delayColor = juce::Colours::yellow;
                else delayColor = juce::Colours::red;
                
                delayLight.setColour(juce::Label::backgroundColourId, delayColor);
                
                // Jitter
                juce::Colour jitterColor;
                if (stats.jitter_buffer_status == 0) jitterColor = juce::Colour(0xff00cc66); // Green
                else if (stats.jitter_buffer_status == 1) jitterColor = juce::Colours::yellow;
                else jitterColor = juce::Colours::red;
    ...
    void toggleFXPanel()
    {
        ...
        if (fxPanelVisible && !fxPanel)
        {
            ...
            fxPanel->onReverbPreDelayChanged = [this](float ms) {
                reverbPreDelay = ms;
            };
            fxPanel->onReverbHPChanged = [this](float hz) {
                reverbHP = hz;
            };
            fxPanel->onReverbLPChanged = [this](float hz) {
                reverbLP = hz;
            };
    ...
    void hideAllDialogs()
    {
        if (connectDialog) removeChildComponent(connectDialog.get());
        if (settingsDialog) removeChildComponent(settingsDialog.get());
    ...
    void updateChannelStrips()
    {
        ...
        for (int i = 0; i < numChannels; ++i)
        {
            ...
            if (jamulus_client_get_channel_info(jamulusClient, i, &info))
            {
                stripsToCreate.push_back({i, info, (info.id == myId)});
            }
        }
        ...
        for (const auto& ci : stripsToCreate)
        {
            ...
            strip->setChannelInfo(ci.info.id, juce::String(ci.info.name), ci.info.instrument, ci.info.country_code, ci.info.skill_level, ci.isMe);
            
            auto stripPtr = strip.get();
            strip->onGainChanged = [this](int id, float gain) {
                jamulus_client_set_channel_gain(jamulusClient, id, gain);
            };
            strip->onPanChanged = [this](int id, float pan) {
            ...
            strip->onSoloChanged = [this](int id, bool solo) {
                jamulus_client_set_channel_solo(jamulusClient, id, solo);
            };
    ...
    void updateSidechainButtonColor()
    {
        if (sidechainButton.getToggleState())
            ...
            sidechainButton.setColour(juce::TextButton::buttonColourId, juce::LookAndFeel::getDefaultLookAndFeel().findColour(juce::TextButton::buttonColourId));
    }
