    ...
    m_pSettings ( pSettings )
{
    ...
    /// @rpc_method jamulusclient/setCurrentDirectory
    /// @brief Selects the current directory, as returned by jamulusclient/getCurrentDirectory and used as the
    ///  default in jamulusclient/pollServerList.
    /// @param {string} params.directory - Socket address of the directory to select, in the same format as the
    ///  strings returned by jamulusclient/getDirectories (and accepted as params.directory by
    ///  jamulusclient/pollServerList). Must be one of the directories returned by jamulusclient/getDirectories.
    /// @result {string} result - "ok" or "Not Found" if the given directory is not one of the known directories.
    pRpcServer->HandleMethod ( "jamulusclient/setCurrentDirectory", [=] ( const QJsonObject& params, QJsonObject& response ) {
        auto jsonDirectory = params["directory"];
        if ( !jsonDirectory.isString() )
        {
            response["error"] = CRpcServer::CreateJsonRpcError ( CRpcServer::iErrInvalidParams, "Invalid params: directory is not a string" );
            return;
        }

        const QString strDirectory = jsonDirectory.toString();

        // built-in directories, in the same order as jamulusclient/getDirectories
        for ( int i = AT_DEFAULT; i < AT_CUSTOM; i++ )
        {
            if ( NetworkUtil::GetDirectoryAddress ( static_cast<EDirectoryType> ( i ), "" ) == strDirectory )
            {
                m_pSettings->eDirectoryType        = static_cast<EDirectoryType> ( i );
                m_pSettings->iCustomDirectoryIndex = 0;
                response["result"]                 = "ok";
                return;
            }
        }
        ...
        {
            if ( !m_pSettings->vstrDirectoryAddress[i].isEmpty() && m_pSettings->vstrDirectoryAddress[i] == strDirectory )
            {
                m_pSettings->eDirectoryType        = AT_CUSTOM;
                m_pSettings->iCustomDirectoryIndex = i;
                response["result"]                 = "ok";
                return;
            }
        }
