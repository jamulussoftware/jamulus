...
CAsioDriverInfo::~CAsioDriverInfo() { close(); }
...
LONG CAsioDriverInfo::open()
{
    ...
    */
    ...
    else
    {
        ...
        case REGDB_E_CLASSNOTREG: // A specified class is not registered in the registration database.Also can indicate that the type of server you
                                  // requested in the CLSCTX enumeration is not registered or the values for the server types in the registry are
                                  // corrupt.
            return DRVERR;
        case E_NOINTERFACE: // The specified class does not implement the requested interface, or the controlling IUnknown does not expose the
                            // requested interface.
            return DRVERR;
...
CAsioDriverInfoList::CAsioDriverInfoList() : lNumDrivers ( 0 ), pDrvListRoot ( (CAsioDriverInfo*) NULL ), pCurrentDriver ( (CAsioDriverInfo*) NULL )
{}

CAsioDriverInfoList::~CAsioDriverInfoList() { deleteDrvList(); }
...
CAsioDrivers::CAsioDrivers() : CAsioDriverInfoList(), bInitialised ( false ) {}
