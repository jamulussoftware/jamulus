...
private:
    void timerCallback() override
    {
        ...
        if ( delay >= 0 )
        {
            ...
            // Color code based on latency thresholds (like original Qt GUI)
            juce::Colour delayColor;
            if ( delay <= 43 )
                delayColor = juce::Colour ( 0xff00cc66 ); // Green - excellent
            else if ( delay <= 68 )
                delayColor = juce::Colour ( 0xffffa500 ); // Orange - warning
            else
                delayColor = juce::Colours::red; // Red - bad
