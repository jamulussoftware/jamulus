                                  ...
                                  const int iNumClients )
{
         ...
         !DoubleFrameSizeConvBufIn[iCurChanID].Get ( vecvecsData[iChanCnt], SYSTEM_FRAME_SIZE_SAMPLES * vecNumAudioChannels[iChanCnt] ) )
    {
        ...
        {
            // get data
            const EGetDataStat eGetStat = vecChannels[iCurChanID].GetData ( vecvecbyCodedDataIn[iChanCnt], iCeltNumCodedBytes );
            ...
            {
                pCurCodedData = &vecvecbyCodedDataIn[iChanCnt][0];
            }
            else
            {
