...
protected:
    ...
    virtual void Init();
    virtual void ProcessSndCrdAudioData ( CVector<short>& vecsStereoSndCrd );
    virtual void ProcessAudioDataIntern ( CVector<short>& vecsStereoSndCrd );

    int  PreparePingMessage();
    int  EvaluatePingMessage ( const int iMs );
    void CreateServerJitterBufferMessage();

    virtual void ClearClientChannels();
    virtual void FreeClientChannel ( const int iServerChannelID );
    virtual int  FindClientChannel ( const int iServerChannelID, const bool bCreateIfNew ); // returns a client channel ID or INVALID_INDEX
    virtual bool ReorderLevelList ( CVector<uint16_t>& vecLevelList );                      // modifies vecLevelList, passed by reference
