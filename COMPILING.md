...
**Note:** the official macOS builds use Qt6, plus Qt5 for the legacy Intel build; for the exact versions, check `.github/autobuild/mac-dependencies_qt5.sh` or `.github/autobuild/mac-dependencies_qt6.sh`. The steps below use Qt5 from Homebrew because it is easier to set up. To install Qt6, use the [official open-source installer](https://www.qt.io/download-qt-installer).
...
**Note:** In order to run the application, you need to run `macdeployqt ./Release/Jamulus.app` once to set up all required libraries and frameworks.
